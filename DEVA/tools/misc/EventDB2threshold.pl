...
# TrecVid Multimedia Event Detection EventDB to Threshold
#
...
my $versionid = "TrecVid Event Detection EventDB to Threshold Version: $version";
...
my %eventids = $icsvh->loadCSV_tohash($if, $eid);
MMisc::error_quit("Problem with input CSV: " . $icsvh->get_errormsg()) if ($icsvh->error());
...
my @ocsv = ($eid, 'DetectionThreshold', 'DetectionTPT', 'EAGTPT');
...
foreach my $event (sort keys %eventids) {
  ...
  @ocsv = ($event, sprintf("%.03f", 0.4 + &get_rand(0.6)), sprintf("%.02f", 10.0 + &get_rand(40)), sprintf("%.02f", &get_rand(20)));
  $otxt .= $ocsvh->array2csvline(@ocsv) . "\n";
  MMisc::error_quit("Problem with output CSV: " . $ocsvh->get_errormsg()) if ($ocsvh->error());
}
...
Usage: $0 [--help | --version] EventDB.csv threshold.csv
