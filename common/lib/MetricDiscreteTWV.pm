...
my @trials_params = TrialsDiscreteTWV::getParamsList();
...
TrecVid08/lib/MetricDiscreteTWV - Compiled 2-class detection metrics for TRECViD '08 Event Detection
...
sub unitTest {
  ...
  print "Test MetricDiscreteTWV...".(defined($dir) ? " Dir=/$dir/" : "(Skipping DET Curve Generation)")."\n";

  my $trial = new TrialsDiscreteTWV({ ("TotTrials" => 40, "IncludeBlocksWithNoTargets" => 1) });
  my $met = new MetricDiscreteTWV({ ('Cost' => 0.1, 'Value' => 1, 'Ptarg' => 0.0001 ) }, $trial);
  ...
  if (defined($dir)){
    ...
    print $ds->renderAsTxt("$dir/DiscreteTWV.unitTest.test2.det", 1, 1, $options, "");                                                                     
  }   
  ...
  my $met2 = new MetricDiscreteTWV({ ('Cost' => 0.1, 'Value' => 1, 'Ptarg' => 0.0001 ) }, $trial);
  print "OK\n";
