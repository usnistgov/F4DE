...
my %error_msgs =
  (
   # from 'new'
   "NoParameterToNew"  => "No parameters allowed on \'new\' call",
...
sub new {
  ...
  $errormsg = $error_msgs{"NoParameterToNew"} if (scalar @_ > 0);
...
sub _fs_check_and_optimize_value {
  ...
  # Check the value
  ($value, $errormsg) = &_fs_check_value($value);
...
sub set_value {
  ...
  my ($value, $errormsg) = _fs_check_and_optimize_value($tmp);
  if ($errormsg !~ m%^\s*$%) {
...
sub check_if_overlap {
  ...
  return (0, 1) if ($self->_is_errormsg_set());
  ...
  # No overlap possible
  return(0, 0) if (($c_end < $i_beg) || ($i_end < $c_beg));
...
sub unit_test { # Xtreme coding and us ;)
  ...
  # No set on new
  my $in0 = "1:3";
  my $fs_tmp0 = new Framespan($in0);
  my $err0 = $fs_tmp0->get_errormsg();
  $otxt .= "$eh Error while checking \'new\' ($err0). "
    if ($err0 ne $error_msgs{"NoParameterToNew"});

  # Now let us try to set a bad value
  my $fs_tmp1 = new Framespan();
  $fs_tmp1->set_value("Not a framespan");
  ...
  my $in3 = "5:4";
  my $fs_tmp3 = new Framespan();
  $fs_tmp3->set_value($in3);
