...
my $versionid = "TrecVid08 Scorer ($versionkey)";
...
if (scalar @inputAliCSV == 0) {
  ...
  my $kernel = new TrecVid08KernelFunctions();
...
sub do_alignment {
  ...
  foreach my $file (@todo) {
    ...
    foreach my $evt (TrecVid08ViperFile::sort_events(@listed_events)) {
      ...
      my $bpm = new BipartiteMatch(\%ref_bpm, \%sys_bpm, \&TrecVid08KernelFunctions::kernel_function, \@kp);
